	...
	public CommandLineRunner initializeDb(ItemRepo itemRepo, RoomRepo roomRepo, AddressRepo addressRepo, BuildingRepo buildingRepo, StudentRepo studentRepo, OrganizationRepo organizationRepo, ItemImageRepo imageRepo, SettingsRepo settingsRepo) {
		return (args) -> {
			...
			uriComponents = UriComponentsBuilder.newInstance()
					.scheme("http").host("localhost:8080").path("/items/"+item1.getId()).build();
			...
			item2.setColor("red");
			uriComponents = UriComponentsBuilder.newInstance()
					.scheme("http").host("localhost:8080").path("/items/"+item2.getId()).build();
			...
			uriComponents = UriComponentsBuilder.newInstance()
					.scheme("http").host("localhost:8080").path("/items/"+item3.getId()).build();
			...
			item4.setColor("blue");
			uriComponents = UriComponentsBuilder.newInstance()
					.scheme("http").host("localhost:8080").path("/items/"+item4.getId()).build();
			...
			uriComponents = UriComponentsBuilder.newInstance()
					.scheme("http").host("localhost:8080").path("/items/"+item5.getId()).build();
